...
public class FeedForwardCharacterizationStepped extends CommandBase {
  ...
  public FeedForwardCharacterizationStepped(Subsystem drive, boolean forwards,
  ...
  public FeedForwardCharacterizationStepped(Subsystem subsystem,
      boolean forwards, FeedForwardCharacterizationData data,
  ...
  public void execute() {
    if (timer.get() < startDelaySecs) {
      ...
      int stepIndex =
          (int) Math.floor((timer.get() - startDelaySecs) / stepLengthSecs);
      double voltage =
          initialVolts + (stepIndex * stepSizeVolts) * (forwards ? 1 : -1);
      if (isDrive) {
      ...
      if ((timer.get() - startDelaySecs) % stepLengthSecs > stepIgnoreSecs) {
        dataPrimary.add(velocitySupplierPrimary.get(),
            voltageSupplierPrimary.get());
        if (isDrive) {
          dataSecondary.add(velocitySupplierSecondary.get(),
              voltageSupplierSecondary.get());
        }
      }
    }
  }
