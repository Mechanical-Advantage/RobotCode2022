...
public class TwoCargoAutoAndEject extends SequentialCommandGroup {
  private static final Pose2d firstCargoTurnPosition =
      new Pose2d(
  ...
  private static final Pose2d hangarEjectPosition =
      FieldConstants.referenceA.transformBy(
          new Transform2d(new Translation2d(2.0, 1.0), Rotation2d.fromDegrees(-150.0)));
  ...
  private static final Pose2d fenderEjectPosition =
      FieldConstants.fenderA.transformBy(
          new Transform2d(new Translation2d(1.0, 0.0), Rotation2d.fromDegrees(180.0)));
      ...
      ejectCommand =
          sequence(
                  new MotionProfileCommand(
                          ...
                          robotState,
                          0.0,
                          List.of(secondCargoPosition, hangarEjectPosition),
                          0.0,
                          true)
                      .deadlineWith(new RunIntake(IntakeMode.FORWARDS, intake, feeder, leds)),
                  new Shoot(feeder, leds).withTimeout(5.0))
              .deadlineWith(
                  new PrepareShooterPreset(flywheels, hood, feeder, ShooterPreset.HANGAR_EJECT));
    } else {
      ejectCommand =
          sequence(
              ...
              new MotionProfileCommand(
                      ...
                      robotState,
                      0.0,
                      List.of(fenderEjectTurnPosition, fenderEjectPosition),
                      0.0,
                      false)
                  .deadlineWith(new RunIntake(IntakeMode.FORWARDS, intake, feeder, leds)),
              new RunIntake(IntakeMode.BACKWARDS_SLOW, intake, feeder, leds).withTimeout(5.0));
    }
