...
public class ThreeCargoAutoCrossMidline extends SequentialCommandGroup {
  private static final Pose2d firstTurnPosition =
      new Pose2d(
  ...
  private static final Pose2d ejectPosition =
      cargoPosition.transformBy(
          new Transform2d(new Translation2d(-1.45, 0.0), Rotation2d.fromDegrees(0.0)));
   ...
   */
  public ThreeCargoAutoCrossMidline(
    ...
    Supplier<Command> shootOwnCommandSupplier =
        () ->
            shootSequenceSupplier
                .get()
                .deadlineWith(new PrepareShooterAuto(flywheels, hood, null, robotState));
    Supplier<Command> shootOpponentCommandSupplier =
        () ->
            shootSequenceSupplier
                .get()
                .deadlineWith(
                    new PrepareShooterPreset(flywheels, hood, null, ShooterPreset.OPPONENT_EJECT));

    MonitorColor monitorColorCommand = new MonitorColor(feeder);
    Command selectedShootCommand =
        new SelectCommand(
            Map.of(
                false,
                sequence(shootOwnCommandSupplier.get(), shootOpponentCommandSupplier.get()),
                true,
                sequence(shootOpponentCommandSupplier.get(), shootOwnCommandSupplier.get())),
    ...
    addCommands(
        new TwoCargoAuto(
            ...
            leds,
            firstShootSecs),
        new InstantCommand(() -> hood.moveToAngle(cargoTrackingHoodAngle), hood),
        new InstantCommand(() -> cargoTrackingAutoAim.updatePipeline(), hood),
        new TurnToAngleProfile(
            drive,
            robotState,
            TwoCargoAuto.cargoPositions.get(AutoPosition.TARMAC_A).getRotation(),
            firstTurnPosition.getRotation()),
        new MotionProfileCommand(
                drive, robotState, 0.0, List.of(firstTurnPosition, ejectPosition), 0.0, false)
        ...
        new RunIntake(IntakeMode.BACKWARDS_SLOW, intake, feeder, leds).withTimeout(ejectDuration),
        deadline(
            sequence(
                new SelectCommand(
                    Map.of(
                        ...
                        new MotionProfileCommand(
                            ...
                            robotState,
                            0.0,
                            List.of(ejectPosition, collectPositionRed),
                            0.0,
                            false,
                        ...
                        new MotionProfileCommand(
                            ...
                            robotState,
                            0.0,
                            List.of(ejectPosition, collectPositionBlue),
                            0.0,
                            false,
                            List.of(collectConstraint))),
            ...
            new PrepareShooterPreset(flywheels, hood, feeder, ShooterPreset.OPPONENT_EJECT),
            monitorColorCommand),
        new InstantCommand(() -> feeder.requestTowerShootPercent(towerPercent)),
        selectedShootCommand.deadlineWith(
            new StartEndCommand(
                () -> vision.setForceLeds(true), () -> vision.setForceLeds(false), vision)));
  }
