...
public class FiveCargoAuto extends SequentialCommandGroup {
  ...
  public static final Pose2d terminalBackUpPosition =
      FourCargoAuto.terminalCargoPosition.transformBy(
          GeomUtil.transformFromTranslation(-0.75, 0.0));
  ...
  public static final Pose2d thirdShotPosition =
      TwoCargoAuto.calcAimedPose(
          FourCargoAuto.terminalCargoPosition.transformBy(
              GeomUtil.transformFromTranslation(-2.5, 0.5)));
  public static final Pose2d thirdShotApproachPosition =
      thirdShotPosition.transformBy(GeomUtil.transformFromTranslation(0.5, 0.0));
  ...
  public FiveCargoAuto(
    ...
    MotionProfileCommand firstShotToSecondCargo =
        new MotionProfileCommand(
            ...
            robotState,
            0.0,
            List.of(ThreeCargoAuto.tarmacDCTurnPosition, ThreeCargoAuto.tarmacCCargoPosition),
            0.0,
            false);
    MotionProfileCommand secondCargoToSecondShot =
        new MotionProfileCommand(
            ...
            robotState,
            0.0,
            List.of(ThreeCargoAuto.tarmacCCargoPosition, ThreeCargoAuto.tarmacCShootPosition),
            0.0,
            true);
    ...
    MotionProfileCommand terminalToThirdShot =
        new MotionProfileCommand(
            ...
            List.of(
                FourCargoAuto.terminalCargoPosition,
                terminalBackUpPosition,
                thirdShotApproachPosition,
                thirdShotPosition),
            0.0,
            true,
            List.of(terminalBackUpConstraint));

    // Full driving seqeuence, including waits
    double terminalArrivalTime =
        startToFirstCargo.getDuration()
            + firstShotLateSecs
            + firstShotDurationSecs
            + firstShotToSecondCargoTurn.getDuration()
            + firstShotToSecondCargo.getDuration()
            + secondCargoToSecondShot.getDuration()
            + secondShotLateSecs
            + secondShotDurationSecs
            + secondShotToTerminal.getDuration();
    double terminalLeaveTime = endTime - thirdShotDurationSecs - terminalToThirdShot.getDuration();
    ...
    Command shootSequence =
        sequence(
            ...
            new Shoot(feeder, leds)
                .alongWith(
                    new RunIntake(
                        IntakeMode.FORWARDS, intake, feeder, leds, true)) // Continue rollers
                .withTimeout(firstShotDurationSecs),
            new RunIntake(IntakeMode.FORWARDS, intake, feeder, leds)
                .withTimeout(secondShotStart - firstShotStart - firstShotDurationSecs),
            new Shoot(feeder, leds).withTimeout(secondShotDurationSecs),
            new RunIntake(IntakeMode.FORWARDS, intake, feeder, leds)
                .withTimeout(thirdShotStart - secondShotStart - secondShotDurationSecs),
            new Shoot(feeder, leds).withTimeout(thirdShotDurationSecs));

    // Prepare shooter sequence, runs in parallel
    Command prepareShooterSequence =
        sequence(
            new PrepareShooterAuto(
                    flywheels,
                    hood,
                    feeder,
                    TwoCargoAuto.cargoPositions.get(AutoPosition.TARMAC_D).getTranslation())
                .withTimeout(firstShotStart + firstShotDurationSecs),
            new PrepareShooterAuto(
                    flywheels, hood, feeder, ThreeCargoAuto.tarmacCShootPosition.getTranslation())
                .withTimeout(
                    secondShotStart
                        + secondShotDurationSecs
                        - firstShotStart
                        - firstShotDurationSecs),
            new PrepareShooterAuto(flywheels, hood, feeder, thirdShotPosition.getTranslation()));
    ...
    Command ledSequence =
        sequence(
            new WaitCommand(alertStart),
            new StartEndCommand(() -> leds.setAutoAlert(true), () -> leds.setAutoAlert(false))
                .withTimeout(alertDuration));

    // Combine all commands
    addCommands(
        new InstantCommand(intake::extend, intake),
        deadline(parallel(driveSequence, shootSequence), prepareShooterSequence, ledSequence),
        new PrintCommand(
            String.format(
